...
double fermi_density_deri(double x, double kT, double mu){
    if (kT < 1.E-12)
        cout << "Temperature too low the compute fermi_density_deri()!" << endl;

    double alpha = (x-mu)/abs(kT);
    if (abs(alpha) > 20) {
        return 0;
    }
    else {
        return -1/kT * exp(alpha) / pow(exp(alpha) + 1.0, 2);
    }
}

double Lorentzian(double x, double eta){
    return 1/M_PI * eta / (pow(x, 2) + pow(eta, 2));
}
